...
import pcl.mud.OpenUpdater;
import pcl.mud.UpdateChecker;
import pcl.mud.UpdateEntry;
...
public class GuiChangelogDownload extends GuiScreen
{
    ...
    public GuiChangelogDownload(GuiScreen parent){
        ...
        if(!OpenUpdater.hasChecked){
            new UpdateChecker(OpenUpdater.getAllUpdateEntries()).run();
        }

        this.entries=new ArrayList<UpdateEntry>(OpenUpdater.getAllUpdateEntries());
    }
    ...
    public void initGui()
    {
        ...
        disable = new GuiButton(3, 15, 10, 125, 20, StatCollector.translateToLocal("mud.disable")+": "+Boolean.toString(!OpenUpdater.enabled));
            ...
            if(!isDownloading || (downloadFailed || downloadComplete) ){
                switch (button.id)
                {
                    case 3:
                        isDownloading = false;
                        close1.enabled = true;
                        OpenUpdater.toggleState();
                        disable.displayString = StatCollector.translateToLocal("mud.disable")+":"+Boolean.toString(!OpenUpdater.enabled);
                        return;
                    case 4:
    ...
    private class Downloader implements Runnable{
        ...
        private File getJarFile() throws FileNotFoundException {
            String path = GuiChangelogDownload.class.getResource(GuiChangelogDownload.class.getSimpleName() + ".class").getFile();
            if(path.startsWith("/")) {
        ...
        public java.io.File getJarFile(Object obj) throws Exception {
            return getJarFile(obj.getClass());
        }
         
        public java.io.File getJarFile(Class _class) throws Exception {
            ...
            try {
                ...
                if(downloadComplete &&
                    ...
                    if(!orginial.delete()){
                        ...
                        File file = getJarFile(this);
                        String cmd = "java -classpath \""+file.getAbsolutePath()+"\" pcl.mud.utils.FileDeleter \""+orginial.getAbsolutePath()+"\"";
